	...
	?>
		...
		<div class="wrap">
            ...
            <h2 class="nav-tab-wrapper">
            	<a href="options-general.php?page=restrict-categories" class="nav-tab <?php echo ( $roles === true ) ? 'nav-tab-active' : ''; ?>">Roles</a>
                <a href="options-general.php?page=restrict-categories&amp;type=users" class="nav-tab <?php echo ( $users === true ) ? 'nav-tab-active' : ''; ?>">Users</a>
            </h2>
...
class RestrictCats_Walker_Category_Checklist extends Walker {
	...
	function start_el(&$output, $category, $depth, $args) {
		...
		'<label class="selectit"><input value="' . $category->slug . '" type="checkbox" name="' . $options_name . '['. $admin .'][]" ' . checked( in_array( $category->slug, $selected_cats ), true, false ) . ' /> ' . esc_html( apply_filters('the_category', $category->name )) . '</label>';
	}
