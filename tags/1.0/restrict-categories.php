...
Description: Restrict the categories that users in defined roles can view and edit in the admin panel.
Author: Matthew Muro
Version: 1.0
*/
...
function RestrictCats_init() {
	...
	register_setting( $rc_shortname.'_options_group', $rc_shortname.'_options' );
}
	...
	if ( $_GET['page'] == plugin_basename(__FILE__) ) {
		...
		if ( 'save' == $_REQUEST['action'] ) {
			...
			foreach ( $rc_options as $value ) {
				...
				$_REQUEST[ $key ] = implode( ',', $_REQUEST[ $key ] ); //This will take from the array and make one string
				...
				<fieldset>
					...
					<div style="overflow:hidden; margin:0 0 5px; float:left; width: 25%;">
                        <label for="<?php echo $value['id'] . '-' . $option; ?>">
                            <input id="<?php echo $value['id'] . '-' . $option; ?>" name="<?php echo $value['id']; ?>[]" type="checkbox" value="<?php echo $option; ?>" <?php echo $checked; ?>/> <?php echo $option; ?>
                        </label>
					</div>
					<?php } ?>
...
function RestrictCats_posts() {
	global $wp_query, $current_user, $rc_shortname;
	...
	foreach ( $user_cap as $key => $value ) {
		...
		$categories = explode( ',', $settings[$key . '_cats'] );
